	...
	for(;;){
		...
		switch(IP->op){
            ...
            case OP_ADD_F:
				...
				fBefore=popd();
				pushd(fBefore+fTop);
				printf("ADD.f\t// %lg+%lg -> %lg",fBefore,fTop,fBefore+fTop);
				IP=IP->next;
				break;
			case OP_FPLOAD_F:
				v=FP[IP->arg.i];
				pushv(v);
				printf("FPLOAD\t%d\t// i:%lg, f:%g",IP->arg.i,v.i,v.f);
				IP=IP->next;
				break;
			case OP_FPSTORE_F:
				v=popv();
				FP[IP->arg.i]=v;
				printf("FPSTORE\t%d\t// i:%lg, f:%g",IP->arg.i,v.i,v.f);
				IP=IP->next;
				break;
			...
			case OP_SUB_F:
				...
				fBefore=popd();
				pushf(fBefore-fTop);
				printf("SUB.f\t// %lg-%lg -> %lg",fBefore,fTop,fBefore-fTop);
				IP=IP->next;
				break;
			case OP_MUL_I:
				iTop=popi();
			...
			case OP_STORE_F:
				...
				*(float*)v.p=fTop;
				pushd(fTop);
				printf("STORE.f\t// *(float*)%p=%lg",v.p,fTop);
				IP=IP->next;
				break;
...
Instr *genTestProgram2(){
	...
	addInstrWithInt(&code,OP_FPLOAD_F,-2);
	addInstr(&code,OP_LESS_F);
	Instr *jfAfter=addInstr(&code,OP_JF);
	// put_d(i);
	addInstrWithInt(&code,OP_FPLOAD_F,1);
